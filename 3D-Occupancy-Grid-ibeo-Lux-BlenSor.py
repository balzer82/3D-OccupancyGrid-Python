...
# 
# [BlenSor](http://blensor.org) is a Laserscanner simulation Toolbox, based on Blender
...
# 
# $$\left[\begin{matrix}x \\ y \\ z \\ 1\end{matrix}\right]_\text{Global} = \left[\begin{matrix} R_{3x3} & t_{1x3} \\ 0 & 1\end{matrix}\right] \cdot \left[\begin{matrix}x \\ y \\ z \\ 1\end{matrix}\right]_\text{Lokal}$$
# 
# wobei $R$ die Rotationsmatrix ist und $t$ der Verschiebungsvektor

# In[54]:

m_start = 320 # Startnummer
pointcloud = pd.DataFrame()

T = np.eye(4)
R = Rypr(0.0, 0.0, 0.0, in_deg=True)
T[:3,:3] = R

for m in range(len(motiondata)):
    # some ibeo lux simulated measurements
    data = pd.read_csv('./scans/scan_%i.pcd' % (m+m_start), delimiter=' ',
    ...
    pcl_global_homogen = np.dot(T, pcl_local_homogen)
    data[['x','y','z']] = pd.DataFrame(pcl_global_homogen[:3, :].T)
                                       
    pointcloud = pointcloud.append(data[['x','y','z']])


# In[52]:

pointcloud.plot(x='x', y='y', c='z', kind='scatter', alpha=0.5, cmap='summer')


# In[53]:

plt3d = plt.figure(figsize=(12, 6)).gca(projection='3d', axisbg='w')
plt3d.scatter(pointcloud.x[::2], pointcloud.y[::2], pointcloud.z[::2], c='r', label='Laserscanner Pointcloud')
#plt3d.scatter(t[0], t[1], t[2], c='k', s=200, label='ibeo Lux')
plt3d.view_init(15, 145)
plt3d.axis('equal')
plt3d.set_xlabel('X')
plt3d.set_ylabel('Y')
